    ...
    <section className=' h-[658px] w-screen max-w-[1366px] flex justify-between items-center'>
      <div className=' w-[53%] h-full flex flex-col justify-center pl-[50px]'>
        <div className='w-[340px] h-[57px] flex justify-around items-center rounded-[10px] bg-gray_1'>
          {
            ['Need Stylist', 'Provide Service'].map(
              (item) => (
                <button key={item} type='button' onClick={
                  () => dispatch(setUiActiveTab(item))
                }
                  className={`${activeTabInState.includes(item[0]) ? 'bg-white' : 'bg-gray_1'
        ...
        <h1 className='mt-[45px] max-w-[620px] ml-[20px] text-[35px] text-gray_5 font-semibold'>
          An
          <span className='h-fit ml-[20px] mr-[20px] pr-2 text-[44px] text-btn_1 font-splash font-[200] transform-gpu rotate-45'>
            individual
          </span>
          lifetime experience anytime, anywhere.
        </h1>
        <h3 className='mt-[40px] max-w-[560px] ml-[20px] text-[26px] text-gray_5 font-[200]'>
          Getting access to Top rated stylists is hard. We do the hard part for you.
        </h3>
      </div >
      <div className=' w-[47%] h-full flex flex-col items-end justify-start ml-[10px]'>
        <div className='relative h-fit pt-24 mb-6 flex items-end justify-start w-full'>
          <div>
            <Image src="/images/hero-1.png" alt="hero-1" width={78} height={78} loading={'lazy'} />
          </div>
          <div className='pb-2 pl-10'>
            <Image src="/images/hero-2.png" alt="hero-2" width={36} height={36} loading={'lazy'} />
          </div>
          <div className='-z-10 absolute bottom-[-65px] left-20 min-w-[32px] min-h-[32px]'>
            <Image src="/images/hero-3.png" alt="hero-3" width={64} height={64} loading={'lazy'} />
          </div>
        </div>
